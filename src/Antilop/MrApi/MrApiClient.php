...
class MrApiClient
{
    ...
    public function __construct($url_ws, $site_id, $site_key)
    {
        ...
        static::$url_ws = $url_ws;
        $client = new nusoap_client(static::$url_ws.'Web_Services.asmx?WSDL', true);
        $client->soap_defencoding = 'utf-8';

        static::$client = $client;
        static::$web_site_id = $site_id;
        static::$web_site_key = $site_key;
    }

    protected function setParamsClient($params)
    {
        $this->params_client = array_merge(
            array(
                'Enseigne' => static::$web_site_id
            ),
            $params
        );
        $this->addKeySecurity();
